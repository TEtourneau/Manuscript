...
Nous suspectons la façon dont est calculé le champ $\delta_F$ : comme nous le verrons dans la section~\ref{prov}, nous n'observons aucune déviation de la sorte lorsque nous estimons les corrélations \lya{}$\times$QSO à partir des mocks eboss-0.0 ou eboss-0.2.
...
Comme expliqué dans la section~\ref{prov}, le procédure d'ajustement des mocks nous permet d'obtenir un spectre de puissance à une dimension $P^{\mathrm{1D}}$ correcte dans la gamme $\num{1.8} < z < \num{3.6}$.
