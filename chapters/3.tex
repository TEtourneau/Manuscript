...
\begin{equation}
  ...
  \overline \Lambda_q = \frac{\sum_{\lambda} w_q(\lambda) \Lambda}{\sum_{\lambda} w_q(\lambda)} .
\end{equation}
...
\end{equation}
Du fait que $a_q$ et $b_q$ sont ajustés sur les $f_q(\lambda)$, l'utilisation de l'équation~\ref{eq:diversity2} pour déterminer le continuum force la moyenne et la pente de chaque région spectrale à être nulle.
Afin de prendre en compte les corrélations introduites par cet effet, le champ 
