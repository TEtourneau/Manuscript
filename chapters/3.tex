...
Dans cette section, nous décrivons comment, du flux observé $f_q(\lambda)$ du quasar $q$ à la longueur d'onde observée $\lambda$, nous pouvons déduire les fluctuations du champ d'absorption \lya{}.
Comme présenté dans la section~\ref{subsec:lya}, le contraste de l'absorption \lya{} $\delta_q(\lambda)$ est défini comme
\begin{equation}
  \label{eq:deltaF}
  \delta_q = \frac{f_q(\lambda)}{\overline F(\lambda) C_q(\lambda)} - 1,
\end{equation}
où $\overline F(\lambda)$ est la transmission moyenne au redshift $z = \lambda_{\mathrm{Ly}\alpha} / \lambda - 1$, et $C_q(\lambda)$ donne le flux du spectre sans absorption. Il est dénommé \emph{continuum} et est différent pour chaque quasar.
Le terme $\overline F(\lambda) C_q(\lambda)$ représente donc le flux moyen attendu du quasar $q$.
Du fait de la faible résolution des données, $\overline F(\lambda)$ n'est pas mesurable. Pour ne pas dépendre d'analyses externes, on détermine directement le produit $\overline F(\lambda) C_q(\lambda)$ pour chaque spectre. Ce calcul, opéré de manière itérative, est décrit dans les lignes qui suivent.

Premièrement, la diversité dans les spectres est modélisée par une relation linéaire en $\mathrm{log}_{10} \lambda$ :
\begin{equation}
  \overline F(\lambda) C_q(\lambda) = \overline C(\lambda_{\mathrm{RF}})(a_q + b_q \log_{10}(\lambda)),
\end{equation}
où $\overline C(\lambda_{\mathrm{RF}})$ donne le continuum moyenné sur tous les spectres, en fonction de la longueur d'onde dans le référentiel du quasar. Les termes $a_q$, $b_q$ et $\overline C(\lambda_{\mathrm{RF}})$ sont déterminés en minimisant la fonction de vraissemblance
\begin{equation}
  \label{eq:likelihood}
  - 2 \ln L = - \sum_{i} \frac{\left[f_{i} - \overline F C_q(\lambda_{i}, a_q, b_q)\right]^2}{\sigma_q^2 (\lambda_i)} + \ln [\sigma_q^2 (\lambda_i)] .
\end{equation}
...
Premièrement, les 5 paramètres à ajuster, ainsi que $\overline C(\lambda_{\mathrm{RF}}$, sont initialisés. Ceci permet d'ajuser $\overline F C_q$ sur $f_q(\lambda)$ en minimisant la fonction de vraissemblance $L$. Dans cet ajustement, chaque pixel $i$ est pondéré par un poid
\begin{equation}
  \label{eq:weights}
  w_{q, i}(\lambda) = \frac{1}{\sigma_{q, i}^2(\lambda)} .
\end{equation}
Une fois cette ajustement fait, $\overline C(\lambda_{\mathrm{RF}}$ est de nouveau calculé. Puis, les paramètres $\eta(\lambda)$, $\sigma_{\mathrm{cosmo}}(\lambda)$ et $\epsilon(\lambda)$ sont ajustés de manière à ce que, à lambda fixé, $\sigma_q(\lambda)$ reproduise la variance estimée à partir des flux $f_q(\lambda)$ de tous les quasars. Ce nouvel ajustement permet une nouvelle fois d'ajuster $\overline F C_q$, etc. Cette procédure est répétée jusqu'à attendre une convergence, au bout de 5 itérations.
