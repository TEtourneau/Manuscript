...
Du fait que, pour le modèle C-G, le terme $F_{\textsc{HCD}}$ soit calculé à partir de la distribution en $\log n_{\textsc{HI}}$ des HCD présents, nous pouvons envisager d'ajuster avec ce modèle des mocks avec différentes distributions de HCD : les mocks eboss-0.2 où tous les HCD sont présents ($\num{17.2} < \log n_{\textsc{HI}} < \num{22.5}$), les mocks eboss-0.2 où les HCD avec $\log n_{\textsc{HI}} < \num{20.3}$ sont  masqués, ou n'importe quelle autre distribution.
...
\begin{figure}
  ...
  \caption{Mesure des paramètres $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ (gauche) et $\beta_{\mathrm{Ly}\alpha}$ (droite) sur l'auto-corrélation \lya$\times$\lya{} estimée à partir des mocks. Les points bleus présentent la mesure faite sur les mocks eboss-0.0. Les points magenta la mesure faite sur les mocks eboss-0.2, où les HCD avec $\log n_{\mathrm{HI}} > \num{20.3}$ sont masqués dans le calcul des fonctions de corrélation, et ajusté avec le modèle de Rogers. Les points orange la mesure faite sur ces mêmes fonctions de corrélations, mais ajustées avec le modèle C-G. Enfin, les points rouges présentent la mesure faite sur les mocks eboss-0.2 où aucun HCD n'a été masqué, ajustés avec le modèle C-G.
    Les tirets correspondent à l'ajustement de chaque jeu de donnée par une loi de puissance.
...
\end{figure}
Nous remarquons que la mesure des paramètres \lya{} faite avec le modèle C-G sur les mocks eboss-0.2 où les HCD sont masqués (orange) est compatible avec la mesure faite sur les mocks eboss-0.2 où les HCD ne sont pas masqués. Ceci est encourageant car cela signifie que le modèle C-G est cohérent avec lui même.
...
Enfin, les mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ faites avec le modèle C-G sur les mocks eboss-0.2 sont en meilleur accord avec la mesure faite sur les mocks eboss-0.0 que la mesure faite avec le modèle de Rogers sur les mocks eboss-0.2.
...
La figure~\ref{fig:cf_models_no_dist} donnent ces mêmes modèles non multipliés par la matrice de distorsion.
  En comparant les courbes oranges aux courbes vertes, nous pouvons remarquer sur ces figures que l'effet des HCD est assez semblable à celui d'une augmentation du biais.
...
\begin{figure}
  ...
  \caption{Les différentes composantes du modèle ajusté sur l'auto-corrélation \lya{}$\times$\lya{} des données DR16. Elles sont décrites dans la légende de la figure~\ref{cf_models}.
...
Pour chaque valeur de $\log n_{\mathrm{HI}}$, nous choisissons une valeur de $L_{\textsc{HCD}}$ qui reproduise au mieux le comportement de $F_{\textsc{HCD}}(k)$ dans la gamme $k \lesssim \SI{0.8}{\h\per\Mpc}$. % c'est à dire pour des échelles supérieures à $r \sim \SI{10}{\perh\Mpc}$.
...
Pour chaque valeur de $\log n_{\mathrm{HI}}$, les fonctions $F_{\textsc{HCD}}$ sont similaires dans les deux modèles.
...
Cependant ces mesures ne sont pas compatibles avec les mesures faites sur les mocks eboss-0.0.
...
Nous pouvons premièrement remarquer que, comme pour les mocks, la mesure faite avec le modèle C-G lorsque les HCD avec une densité de colonne $\log n_{\mathrm{HI}} > \num{20.3}$ sont masqués (orange) est très similaires à la mesure faite avec le modèle de Rogers lorsque nous utilisons $L_{\textsc{HCD}} = \SI{2.8}{\perh\Mpc}$ (bleu). Cependant, contrairement aux mocks, ces deux mesures ne sont pas compatibles avec la mesure faite avec le modèle C-G lorsqu'aucun HCD n'est masqué (vert). A titre de comparaison, la mesure faite avec le modèle de Rogers avec $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$ (utilisée comme référence pour construire les mocks) est montrée en noir.
Le fait que les deux mesures faites avec le modèle C-G (HCD masqués ou non) ne soient pas compatibles, alors que les mêmes mesures faites sur les mocks le sont, pose un sérieux problème. Cela signifie que quelque chose dans les données ne se comporte pas de la même manière que dans les mocks. Il se pourrait qu'un effet inconnu, présent dans les données, soit pris en compte par la modélisation des HCD. Cependant nous n'avons pour l'instant pas d'intuition concernant la nature de cet effet.
...
Sur cette figure, nous pouvons remarquer que l'ajustement des mocks avec le modèle de Rogers et l'ajustement avec le modèle C-G donnent des mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$, $\beta_{\mathrm{Ly}\alpha}$ et $b_{\textsc{HCD}}$ très proches. De plus, la somme  $b_{\mathrm{Ly}\alpha} + b_{\textsc{HCD}}$ est identique.
...
En ce qui concerne les données, les mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et $b_{\textsc{HCD}}$ en utilisant le modèle de Rogers ou le modèle C-G ne sont pas compatibles. Les sommes  $b_{\mathrm{Ly}\alpha} + b_{\textsc{HCD}}$ dans ces deux cas sont compatibles. 
...
% Ceci vient du fait que les paramètres \lya{} sont davantage corrélés avec ceux des HCD. Le modèle C-G a donc davantage de mal à faire la distinction entre la contribution du \lya{} et celle des HCD.
...
\begin{figure}
  ...
  \caption{Mesure des paramètres $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ (haut gauche), $\beta_{\mathrm{Ly}\alpha}$ (haut droite), $b_{\textsc{HCD}}$ (bas gauche) et $b_{\mathrm{Ly}\alpha} + b_{\textsc{HCD}}$ (bas droite) sur l'auto-corrélation \lya$\times$\lya{} estimée à partir des données DR16 et des mocks eboss-0.2. Les HCD avec $\log n_{\mathrm{HI}} > \num{20.3}$ sont masqués dans le calcul des fonctions de corrélation. Les données sont ajustées avec le modèle de Rogers (orange) et le modèle C-G (rouge). Les mocks sont ajustés avec le modèle de Rogers (bleu) et le modèle C-G (vert). Les points verts sont décalés des points bleus de $\Delta z = 5\times10^{-3}$ pour des raisons de visibilité. Idem pour les points rouges et jaunes.}
...
Cependant, ce n'est pas ce que nous observons. Le tableau~\ref{tab:hcd_dndz3} résume ces mesures.
Pour une même version, les mesures de $b_{\textsc{HCD}}$ faites avec le modèle de Rogers ou le modèle C-G ne sont pas compatibles.
Cela est dû à la valeur de $L_{\textsc{HCD}}$ mal choisie pour le modèle de Rogers.
De plus, pour un même modèle, nous remarquons que $b_{\textsc{HCD}}$  mesuré dans la version contenant 3 fois plus de HCD n'est pas compatible avec 3 fois le biais des HCD mesuré dans l'autre version. Le biais des HCD mesuré dans la version contenant 3 fois plus de HCD est sous-estimé.
Ceci a pour effet de réduire la mesure de $\beta_{\mathrm{Ly}\alpha}$.
Nous pouvons toute fois noter que le modèle C-G produit des mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ compatibles dans ces deux versions, ce qui n'est pas le cas du modèle de Rogers.
...
La différence entre la courbe continue bleue et la courbe tiretée verte est très importante. Nous sommes étonnés\footnote{Nous pensons que cela vient du biais des HCD qui est environ \num{2.2} fois supérieur dans la mesure faite avec le modèle C-G. Nous pensons que le biais des HCD plus bas obtenu avec le modèle de Rogers compense les mesures des paramètres \lya{}.} que les ajustements faits sur les mocks eboss-0.2 avec le modèle de Rogers et le modèle C-G produisent des résultats si similaires (voir la figure~\ref{fig:bias_CG_mask}) compte tenu de ces différences sur $F_{\textsc{HCD}}$.
  Dans tous les cas, nous pensons que la mesure faite avec $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$ n'est pas fiable.
La figure~\ref{fig:f_hcd_range} présente aussi les fonctions $F_{\textsc{HCD}}$ obtenues pour la gamme $\num{17.2} < \log n_{\mathrm{HI}} < \num{22.5}$. La valeur de $L_{\textsc{HCD}}$ choisie est \SI{6.5}{\perh\Mpc}. Les fonctions $F_{\textsc{HCD}}$ obtenues pour cette gamme dans le cadre du modèle C-G et du modèle de Rogers (courbes rouges) sont différentes. Le modèle de Rogers atteint ici ses limites.
...
\paragraph{}
Les analyses présentées dans ce chapitre sont présentées dans un ordre chronologique : nous nous sommes rendus compte très tardivement que le modèle utilisé pour modéliser les HCD, à savoir le modèle de Rogers avec $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$, n'était pas correct.
...
% Aussi, la figure présente les fonctions $F_{\textsc{HCD}}$ obtenues pour la distribution $\num{17.2} < \log n_{\mathrm{HI}} \num{20.3}$ (en rose).
% Afin de reproduire le comportement de $F_{\textsc{HCD}}(k)$ dans la gamme $k > \SI{0.3}{\h\per\Mpc}$, nous devons choisir $L_{\textsc{HCD}} = \SI{3}{\perh\Mpc}$, ce qui est loin des \SI{10}{\perh\Mpc} utilisés jusqu'à présent. Ceci explique les différences que nous observons lorsque nous ajustons les mocks ou les données avec le modèle C-G ou avec le modèle de Rogers en utilisant $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$.

% La seconde méthode consiste à calculer, pour chaque valeur de $\log n_{\mathrm{HI}}$, le profil de Voigt correspondant à l'absorption causée par un HCD avec cette densité de colonne. Le détail du calcul est donné dans~\textcite{Rutten2003}.
% Une fois ces profils de Voigt calculés, nous choisissons $L_{\textsc{HCD}}$ comme étant la largeur à mi-hauteur de ces profils.
% Les valeurs de $L_{\textsc{HCD}}$ que nous obtenons de cette manière pour $\log n_{\mathrm{HI}} \in [\num{17.6}\,;\num{18.2}\,;\num{18.8}\,;\num{19.4}\,;\num{20}]$ sont $[\num{2}\,; \num{3}\,; \num{4}\,; \num{8}\,; \num{16}]$.
% Aussi, nous estimons le $L_{\textsc{HCD}}$ effectif correspondant à la distribution $\num{17.2} < \log n_{\mathrm{HI}} \num{20.3}$ : nous calculons la moyenne des largeurs à mi-hauteur pondérée par le nombre de HCD pour chaque densité de colonne (voir figure~\ref{fig:distrib_dla}). Ceci nous donne un $L_{\textsc{HCD}}$ effectif de \SI{11}{\perh\Mpc}, en accord avec $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$ utilisé jusqu'à présent.

% Nous pouvons remarquer que les valeurs de $L_{\textsc{HCD}}$ obtenues avec la première méthode ne sont pas compatibles avec celles obtenues avec la seconde. Nous utilisons les valeurs de $L_{\textsc{HCD}}$ obtenues avec ces deux méthodes pour réaliser les ajustements des mocks contenant des HCD avec une densité de colonne fixe. Nous ajustons aussi ces mocks avec le modèle C-G.
% La mesure des paramètres $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et $\beta_{\mathrm{Ly}\alpha}$ faites avec ces ajustements est présentée sur la figure~\ref{fig:bias_vs_nhi}.
% L'ajustement avec le modèle C-G est montré en rouge. L'ajustement avec le modèle de Rogers et en utilisant $L_{\textsc{HCD}}$ déterminée par la première méthode est donné en orange (que nous appelons Rogers 1). L'ajustement avec le modèle de Rogers et $L_{\textsc{HCD}}$ déterminée avec la seconde méthode est montré en violet (que nous appelons Rogers 2).
% Aussi, nous donnons les paramètres \lya{} mesurées sur la même réalisation, mais sans y avoir ajouté de HCD. Cette mesure est représentée par la line horizontale bleue. Les pointillés bleus donnent les erreurs à $\pm 1 \sigma$.
% Nous pouvons remarquer premièrement que le paramètre $\beta_{\mathrm{Ly}\alpha}$ n'est pas du tout contraint pour les faibles densités de colonne lorsque nous utilisons le modèle C-G et le modèle Rogers 1. Nous pensons que ceci vient du fait que les profils $F_{\textsc{HCD}}$ sont trop étroits dans l'espace réel. Lorsque nous diminuons $r_{\mathrm{min}}$ à \SI{10}{\perh\Mpc} lors de l'ajustement, cela améliore les constraintes sur $\beta_{\mathrm{Ly}\alpha}$.
% Par ailleurs, nous pouvons remarquer que le modèle C-G et le modèle Rogers 1 produisent des mesures très similaires. Ceci est attendu car les fonctions
% $F_{\textsc{HCD}}(k)$ utilisées dans ces deux modèles sont très similaires pour $k > \SI{0.3}{\h\per\Mpc}$, par construction.
% Enfin et surtout, aucun de ces modèles ne produit une mesure en accord avec les paramètres \lya{} mesurés dans les mocks eboss-0.0.
% Si ces modèles ne parviennent pas à distinguer l'effet du \lya{} de l'effet des HCD lorsque ces derniers possèdent une densité de colonne fixe, alors il y a peu d'espoirs qu'ils parviennent à le faire lorsque les HCD sont distribués dans une large gamme en $\log n_{\mathrm{HI}}$.

% \begin{figure}
%   \centering
%   \includegraphics[scale=0.35]{bias_vs_nhi}
%   \caption{Mesures des paramètres $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et $\beta_{\mathrm{Ly}\alpha}$ dans les mocks possédant des HCD avec une colonne de densité fixe. Le détail des modèles utilisés et donné dans le texte.}
%   \label{fig:bias_vs_nhi}
% \end{figure}


% \paragraph{}
% % Le dernier test que nous présentons consiste à fixer les paramètres \lya{} lors de l'ajustement des modèles décrits dans le paragraphe précédent sur les mocks qui possédent des HCD avec une densité de colonne fixe, et ajuster le paramètre $L_{\textsc{HCD}}$.
% Le dernier test que nous présentons consiste à fixer les paramètres \lya{} et ajuster le paramètre $L_{\textsc{HCD}}$ sur les mocks avec les HCD possédant une densité de colonne fixe. Les paramètres \lya{} sont fixées à leur valeur mesurée dans la version eboss-0.0 correspondante.
% % Puis, refaire l'ajustement avec le modèle de Rogers en relâchant les paramètres \lya{} et en utilisant la valeur de $L_{\textsc{HCD}}$ obtenue précédemment.
% Une fois cette valeur de $L_{\textsc{HCD}}$ obtenues, nous réajustons le modèle de Rogers en relâchant les paramètres \lya{} et en utilisant cette nouvelle valeur de $L_{\textsc{HCD}}$.
% La figure~\ref{fig:lhcd_vs_nhi} présente la mesure de $L_{\textsc{HCD}}$ lorsque les paramètres \lya{} sont fixés (points roses). A titre de comparaison, nous représentons les valeurs de $L_{\textsc{HCD}}$ obtenues avec la première et la seconde méthode, et utilisées respectivement dans les modèles Rogers 1 et Rogers 2.
% Nous pouvons remarquer que le modèle de Rogers lorsqu'il est ajusté sur les mocks avec les HCD possédant la plus faible densité de colonne donne des mesures de $L_{\textsc{HCD}}$ bien plus grandes que lorsqu'il est ajusté sur les mocks avec les HCD possédant une grande densité de colonne.
% Cet effet est opposé à ce à quoi nous nous attendons. Nous pensons que ceci provient du fait que le modèle a du mal à distinguer la quantité de HCD (représentée par $b_{\textsc{HCD}}$) de l'amplitude de l'effet des HCD (liée à $\log n_{\mathrm{HI}}$ et donc à $L_{\textsc{HCD}}$). Ceci semble confirmé par l'anticorrélation importante des paramètres $b_{\textsc{HCD}}$ et $L_{\textsc{HCD}}$, mesurée comme valant plus de \SI{70}{\percent} en valeur absolue. Cette anticorrélation dépasse les \SI{85}{\percent} pour $\log n_{\mathrm{HI}} \leq \num{18.8}$.

% \begin{figure}
%   \centering
%   \includegraphics[scale=0.45]{lhcd_vs_nhi}
%   \caption{Evolution du paramètre $L_{\textsc{HCD}}$ en fonction de la densité de colonne. Le détail est donné dans le texte.}
%   \label{fig:lhcd_vs_nhi}
% \end{figure}

% Finalement, nous utilisons les valeurs de $L_{\textsc{HCD}}$ obtenues avec la méthode précédente pour ajuster les mocks avec le modèle de Rogers.
% La figure~\ref{fig:bias_vs_nhi_free_L0} présente les mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et $\beta_{\mathrm{Ly}\alpha}$ produites de cette façon.
% Nous pouvons remarquer que, contrairement au cas des mocks où les HCD sont distribués dans une large gamme en $\log n_{\mathrm{HI}}$ (voir section~\ref{subsec:comparaison_mocks}), le fait d'ajuster au préalable $L_{\textsc{HCD}}$ puis d'utiliser le modèle de Rogers avec le $L_{\textsc{HCD}}$ obtenu pour mesurer les paramètres \lya{} produit des mesures en accord avec les mocks eboss-0.0.
% Cependant, ceci est nuancé par le fait que les barres d'erreurs sont bien plus grande ici que dans le cas de l'ajustement des \Nmocks{} réalisations des mocks eboss-0.2.

% \begin{figure}
%   \centering
%   \includegraphics[scale=0.44]{bias_vs_nhi_free_L0}
%   \caption{Mesures des paramètres $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et $\beta_{\mathrm{Ly}\alpha}$ dans les mocks contenant des HCD avec une densité de colonne fixe (points roses). La mesure des paramètres \lya{} dans les mocks eboss-0.0 est représentée par la line horizontale bleue. Les pointillés donnent les erreurs à $\pm 1 \sigma$.
% Les détails de l'ajustement sont donnés dans le texte.}
%   \label{fig:bias_vs_nhi_free_L0}
% \end{figure}
