...
% des simulations qui visent à reproduire les données d'eBOSS et de DESI. Ces mocks, nommés \texttt{SaclayMocks} et présentés dans \textcite{CITE:mocks}, sont le cœur de ce manuscrit.
...
Ainsi, seuls les \emph{champs aléatoires gaussiens} (GRF pour Gaussian Random Field) permettent de générer un grand volume avec une bonne résolution dans un temps raisonnable, afin de produire un grand nombre de mocks.
...
\end{equation}
% Le champ $\delta_{k, \mathrm{matière}}$ est le champ de fluctuation de la matière à $z = 0$ dans l'espace $k$ calculé précédemment.
% Il est fréquent
...
\textbf{Concernant le champ d'absorption \lya{}, les RSD sont prises en compte par une modification de la formule FGPA (voir la fin de l'appendice~\ref{app:fgpa}). Pour ce faire, nous avons besoin du gradient de vitesse $\eta_{\parallel}$ à $z=0$.
...
\begin{equation}
  \label{eq:eta1}
  \eta_{nm}(\vec k) = - f \frac{k_n k_m}{k^2} \delta_k(\vec k) \; .
\end{equation}
% Cette équation permet de retrouver la formule de kaiser :
\textbf{Ceci nous permet, lorsque nous soustrayons $\eta$ au contraste $\delta$ (appendice~\ref{app:fgpa}), de retrouver la formule de Kaiser :}
\begin{align}
  \label{eq:kaiser5}
...
% \begin{equation}
%   \eta_{ij}(\vec k) = \frac{- k_i k_j}{k^2} f \delta_{k, \mathrm{matière}}(\vec k) \; ; \hspace{0.5cm} (i,j) \in [x, y, z]^2 \; .
% \end{equation}
...
\textbf{Nous pouvons noter ici une importante amélioration par rapport aux mocks développés pour BOSS : ces mocks disposaient de lignes de visée parallèles, ou alors ne disposaient pas de corrélation croisée \lya{}$\times$QSO. Les mocks présentés dans ce manuscrit disposent à la fois de lignes de visée non parallèles et d'une corrélation croisée \lya{}$\times$QSO.}
...
Pour chaque pixel, nous calculons une moyenne pondérée des voxels voisins avec un lissage gaussien.
  ...
  % Nous choisissons un lissage de largeur $\sigma = d_{\mathrm{cell}}$.
  Nous appliquons donc un lissage gaussien, de largeur $\sigma = d_{\mathrm{cell}}$.
% Pour chaque pixel, nous considérons les voxels appartenant au cube de 7 voxels de côté, centré sur le voxel dans lequel se trouve le pixel. Ce cube correspond à considérer les voxels compris à $\pm 3 \sigma$ du voxel central. %, avec $\sigma = d_{\mathrm{cell}}$ (\#prov validé par le plot, demander à JM).
...
% , et enfin $\sigma^2 = d_{\mathrm{cell}}^2$ est la largeur du lissage gaussien appliqué.
...
% \begin{equation}
%   \label{eq:kny}
%   k_{max} = \frac{2 \pi}{1536 d_{\mathrm{cell}}} \sim \SI{1.9e-3}{\h\per\Mpc} \; ,
% \end{equation}
...
Mais cette solution n'est pas envisageable car les lignes de visées ne sont pas parallèles, et nous ne pouvons donc pas confondre l'axe $k_z$ avec l'axe de la ligne de visée $\kpar{}$.
...
Contrairement à la méthode qui utilise le champ de vitesse, la solution qui soustraie le champ $\eta_{\parallel}$ au champ $\delta_l$ permet de prédire les fonctions de corrélation des mocks.
...
\end{equation}
Cette solution produisait une trop faible quantité de RSD. Nous avons alors décidé d'inclure les RSD causées par les petites échelles. Similairement à $\delta_l$ et $\delta_s$, nous soustrayions à la profondeur optique la contribution aux grandes et petites échelles $\eta_l$ et $\eta_s$. En faisant ceci, nous étions plus proche de ce que nous aurions obtenu avec une taille de voxel plus petite.
...
Le champ $\eta_{\parallel}$ est donc soustrait au champ $\delta_l$. Ceci nous permet de retrouver la formule de Kaiser (équation~\ref{eq:kaiser5}). De plus, cette solution ne produit pas de pixel avec $F > 1$.
...
% \begin{equation}
%   \label{eq:gauss_smoothing}
%   % W(k) = \mathrm{e}^{- \frac{k^2 d_{\mathrm{cell}}^2}{2}} \;.
%   W(k) = \exp(- \frac{k^2 d_{\mathrm{cell}}^2}{2}) \;.
% \end{equation}
...
% (\#prov il y a pas un facteur $\frac{1}{2}$ en trop ? on a $\sigma_{smooth}^2 = 2 d_{\mathrm{cell}}^2$)
...
où, cette fois ci, $v_{\textsc{x}}$, $v_{\textsc{y}}$ et $v_{\textsc{z}}$ sont les vitesses le long de la ligne de visée provenant de l'interpolation des boîtes de vitesse (section~\ref{subsec:los_interp}).
...
% Ce choix d'assembler sept boîtes de densité plutôt que d'en utiliser une seule a été contraint par la mémoire maximale des nœuds.
...
% Sur cette machine, nous avons utilisé les nœuds \og Haswell \fg.
...
% Cependant nous n'avions pas l'expertise nécessaire. Nous effectuons donc les transformations de Fourier sur un seul nœud, ce qui explique le découpage du relevé en sept chunks indépendants.
...
En ce qui concerne les paramètres $a$ et $b$, ils sont ajustés afin de retrouver les bons $\overline F(z)$ et $P^{\mathrm{1D}}(k, z)$.
...
%   \sigma_s = \frac{1}{d_{\mathrm{pix}}N} \left( P_{s}(0) + P_{s}(k_{\mathrm{Ny}}) + 2 \sum_{j=1}^{\frac{N}{2} - 1}P_{s}\left(\frac{2 \pi}{d_{\mathrm{pix}}N} j\right)\right) \; ,
% \end{equation}
% où $k_{\mathrm{Ny}} = \pi / d_{\mathrm{pix}} \sim \SI{15.7}{\perh\Mpc}$ est le mode de Nyquist : c'est le mode maximal accessible pour une résolution donnée.
...
% Du fait de la résolution $d_{\mathrm{pix}} = \SI{0.2}{\perh\Mpc}$ des spectres simulés, nous avons besoin de calculer le  $P_{s}$ jusqu'à $k_{\mathrm{Ny}} = \pi / d_{\mathrm{pix}} \sim \SI{15.7}{\perh\Mpc}$.
...
% Celle-ci est mesurée sur une réalisation complète.
