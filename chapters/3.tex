...
% Une deuxième classification, fondée sur le pipeline de SDSS, est alors appliquée \citep{CITE:Lyke in prep}.
% Elle est complétée par l'algorithme \texttt{QuasarNET} \citep{Busca2018} afin de réduire le nombre de mauvaises et non-identifications, et donc le nombre d'inspections visuelles nécessaires. Grâce à \texttt{QuasarNET}, ce chiffre est réduit à \SI{0.6}{\percent} des spectres, soit \num{8581} spectres. Une sous-échantillon du superset est alors construit. Il contient \num{750426} objets, confirmés comme quasar par la chaîne de traitement précédente.
