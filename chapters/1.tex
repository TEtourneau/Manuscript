...
\textbf{Après l'apparition des premières étoiles, le rayonnement émis par celles-ci commence progressivement à réioniser l'hydrogène, qui était devenu neutre au moment de l'émission du CMB. Les dernières analyses estiment le redshift de la réionisation à $5 \lesssim z_{\mathrm{réionisation}} \lesssim 9$~\autocite{Collaboration2018}.
Après la phase de réionisation, la quasi-totalité des atomes d'hydrogène sont ionisés. Seul 1 atome sur \num{10000} à 1 atome sur \num{1000} demeure neutre au sein du milieu intergalactique.}
...
\textbf{Le redshift trouve son origine dans deux effets.
Le premier : le redshift est causé par la vitesse particulière de  l'objet. Dans ce cas, nous parlons d'\emph{effet Doppler}\footnote{L'effet Doppler est l'augmentation ou la diminution de la longueur d'onde d'une onde lorsque l'émetteur de cette dernière s'approche ou s'éloigne de l'observateur. L'exemple le plus connu est celui de l'ambulance : le son entendu est plus aigu lorsque l'ambulance s'approche, puis plus grave lorsqu'elle s'éloigne. Le même processus est à l'œuvre avec la lumière.}.
...
Pour les objets lointains que nous considérons dans ce manuscrit, le redshift induit par les vitesses particulières est négligeable devant le redshift causé par l'expansion de l'univers. Nous confondons donc dans la suite redshift et redshift cosmologique.}

\textbf{Les objets distants s'éloignent de nous du fait de l'expansion. Similairement à l'effet Doppler, le spectre observé de ces objets est décalé vers les grandes longueurs d'onde.
...
\begin{equation}
  ...
  1 + z = \frac{1}{a(t)} \; .
\end{equation} 
\textbf{Le redshift (cosmologique) est donc directement dû à l'expansion de l'univers.}
...
\paragraph{}
\textbf{Les premières observations du premier pic acoustique dans le CMB ont été faites à la fin des années~90. Ces observations ont été menées à l'aide de ballons volant en haute atmosphère (première observation par les expériences QMAP et MAT/TOCO~\autocite{Miller2001} et confirmée par les expériences BOOMERanG~\autocite{Masi2002} et MAXIMA~\autocite{Hanany2000}).
...
Peu de temps après, l'observation des galaxies locales avec le projet SDSS permettait la première détection du pic BAO dans la distribution de matière~\autocite{Eisenstein2005}.}
...
\textbf{En dessous d'un certain seuil de densité, les galaxies ne peuvent se former\footnote{Pour être exact, la probabilité qu'une galaxie se forme dans une région où la densité moyenne est inférieure à ce seuil est extrêmement faible.}.
...
La distribution reconstruite sera donc plus structurée que la distribution sous-jacente : les surdensités seront mieux échantillonnées que les sous-densités.}
...
et $\Delta z_{\mathrm{v}}$ et le redshift induit par la vitesse particulière $v_{\parallel}$ du traceur le long de la ligne de visée (effet Doppler) :
\begin{equation}
  \label{eq:delta_z}
  \Delta z_{\mathrm{v}} = \frac{v_{\parallel}}{c} \; .
\end{equation}

\textbf{Comme nous l'avons vu dans la section~\ref{par:redshift}, l'effet Doppler induit par les vitesses particulières des traceurs s'ajoute au redshift cosmologique.
...
\textbf{Le biais des quasars est mesuré par \textcite{Laurent2017} dans quatre bins en redshift (voir table~2).
Nous utilisons leurs valeurs pour paramétrer le biais des quasars selon une loi de puissance. La paramétrisation que nous utilisons dans ce manuscrit est la suivante}
...
\textbf{Cependant, la façon dont sont conçus les spectrographes, et plus précisément les pixels des capteurs CCD, rendent ce profil discret.}
...
\textbf{Dans les lignes qui suivent, nous décrivons comment relier la densité d'hydrogène neutre le long de la ligne de visée à la quantité de flux absorbé.
Nous négligeons ici et dans la suite de ce manuscrit l'élargissement thermique des raies d'absorption \lya{}, la source dominante d'élargissement des ces profils d'absorption, car cet effet est environ cinq fois inférieur à la résolution des données que nous traitons.}
...
\end{equation}
est la profondeur optique de l'absorbeur. Plus l'absorbeur est dense, plus la profondeur optique $\tau$ est importante et plus la fraction de flux transmis $F$ est faible. Lorsque tout le flux est absorbé, $F = 0$. A l'inverse, lorsque le flux n'est pas absorbé, $F = 1$. \textbf{L'équation~\ref{eq:tau} est dénommée approximation FGPA (Fluctuating Gunn Peterson Approximation, voir section~\ref{subsubsec:lya_field}).}
...
Ce profil d'absorption peut être modélisé par un profil de Voigt.
