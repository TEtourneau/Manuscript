...
\textbf{Comme expliqué dans la section~\ref{subsec:stab_pars_lya}, le masquage des HCD dans les données s'effectue selon le résultat de l'agorithme d'identification. Les HCD identifiés puis reconstruits avec $\log n_{\textsc{HI}} > \num{20.3}$ sont masqués. Dans le cas des mocks, les HCD sont masqués à partir du ``vrai'' catalogue. Nous étudions ici l'effet du masquage à partir du catalogue produit par l'algorithme d'identification.}
...
Pour ce faire, à partir de la même réalisation des mocks, nous avons produit cinq versions eboss-0.2. Dans chacune de ces versions, nous ajoutons les HCD avec une valeur de $\log n_{\mathrm{HI}}$ fixe. Ces valeurs pour les six versions sont $\log n_{\mathrm{HI}} \in [\num{17.6}\,;\num{18.2}\,;\num{18.8}\,;\num{19.4}\,;\num{20}]$. Les nombres relatifs de HCD entre ces versions suit la distribution en $\log n_{\mathrm{HI}}$ utilisée pour construire le catalogue standard de HCD (présentée sur la >figure~\ref{fig:distrib_dla}). La distribution en $z$ utilisée est trois fois plus importante que dans le catalogue standard afin d'avoir suffisamment de HCD.

La figure~\ref{fig:cf_nhi_bins} présente les fonctions de corrélation \lya{}$\times$\lya{} pour les différentes versions des mocks décrites précédemment. Pour simplifier les comparaisons, nous n'utilisons pas de bins en redshift, les fonctions de corrélation sont donc calculées sur l'ensemble des quasars. La ligne bleue foncée donne la corrélation pour les mocks eboss-0.0 (sans HCD). La ligne violette donne la corrélation pour les mocks eboss-0.2 incluant les HCD avec $\num{17.2} < \log n_{\mathrm{HI}} < \num{20.3}$. Les autres couleurs donnent les corrélations pour les mocks eboss-0.2 incluant des HCD avec une valeur fixe en $\log n_{\mathrm{HI}}$.
Nous pouvons voir sur cette figure que les HCD qui ont le plus grand effet sur la fonction de corrélation sont les HCD avec une grande densité de colonne, malgé leur nombre plus restreint. L'effet causé par les HCD avec une densité de colonne de \num{17.6}, \num{18.2}et \num{18.8} est similaire : la faible densité de colonne est compensée par le nombre plus important de HCD.

\begin{figure}
  \centering
  \includegraphics[scale=0.35]{cf_nhi_bins}
  \caption{Fonctions de corrélation \lya{}$\times$\lya{} estimée à partir d'une réalisation. Chaque couleur donne la densité de colonne $\log n_{\mathrm{HI}}$ utilisée pour générer les HCD. Le détail est donné dans le texte.
...
\paragraph{}
Nous étudions maintenant comment le modèle de Rogers et le modèle C-G se comportent lorsque nous ajustons les mocks contenant des HCD avec une densité de colonne fixée.
Dans chacun des ajustements, nous devons utilser pour chaque modèle un terme $F_{\textsc{HCD}}$ adéquat aux HCD présents.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{f_hcd}
  \caption{Les fonctions $F_{\textsc{HCD}}$ utilisées dans le cadre du modèle C-G (lignes continues) ou du modèle de Rogers (lignes tiretées) pour différentes valeurs de $\log n_{\mathrm{HI}}$.}
  \label{fig:f_hcd}
\end{figure}

Nous déterminons $L_{\textsc{HCD}}$ de deux façons différentes. La première consiste à choisir, pour chaque valeur de $\log n_{\mathrm{HI}}$, une valeur de $L_{\textsc{HCD}}$ qui reproduise le comportement de $F_{\textsc{HCD}}(k)$ dans la gamme $k > \SI{0.3}{\h\per\Mpc}$, c'est à dire pour des échelles supérieures à $r \sim \SI{10}{\perh\Mpc}$. Les valeurs de $L_{\textsc{HCD}}$ que nous obtenons pour $\log n_{\mathrm{HI}} \in [\num{17.6}\,;\num{18.2}\,;\num{18.8}\,;\num{19.4}\,;\num{20}]$ sont $[\num{0.28}\,; \num{0.60}\,; \num{1.2}\,; \num{2.4}\,; \num{4.8}]$.
La figure~\ref{prov} présente les fonctions $F_{\textsc{HCD}}(k)$ ainsi obtenus. Les lignes continues donnent les fonctions $F_{\textsc{HCD}}(k)$ calculées par \texttt{picca}. Ce sont les fonctions utilisées dans le cadre du modèle de C-G. Les lignes tiretées donnent les termes $F_{\textsc{HCD}}(k)$ pour le modèle de Rogers, c'est à dire $F_{\textsc{HCD}}(k) = \exp(- L_{\textsc{HCD}} k)$. Pour chaque valeur de $\log n_{\mathrm{HI}}$, les fonctions $F_{\textsc{HCD}}$ sont similaires dans les deux modèles, par construction.
Aussi, la figure présente les fonctions $F_{\textsc{HCD}}$ obtenues pour la distribution $\num{17.2} < \log n_{\mathrm{HI}} \num{20.3}$ (en rose).
Afin de reproduire le comportement de $F_{\textsc{HCD}}(k)$ dans la gamme $k > \SI{0.3}{\h\per\Mpc}$, nous choisissons $L_{\textsc{HCD}} = \SI{3}{\perh\Mpc}$, ce qui est loin des \SI{10}{\perh\Mpc} utilisés jusqu'à présent. Ceci explique les différences que nous observons lorsque nous ajustons les mocks ou les données avec le modèle C-G ou avec le modèle de Rogers en utilisant $L_{\textsc{HCD}} = \SI{10}{\perh\Mpc}$.

La seconde méthode consiste à calculer, pour chaque valeur de $\log n_{\mathrm{HI}}$, le profil de Voigt correspondant à l'absorption causée par un HCD avec cette densité de colonne.
...
Aussi, nous estimons le $L_{\textsc{HCD}}$ effectif correspondant à la distribution $\num{17.2} < \log n_{\mathrm{HI}} \num{20.3}$ : nous calculons la moyenne des largeurs à mi-hauteur pondérée par le nombre de HCD pour chaque densité de colonne (voir figure~\ref{fig:distrib_dla}). Ceci nous donne un $L_{\textsc{HCD}}$ effectif de \SI{11.5}{\perh\Mpc}.
...
$F_{\textsc{HCD}}(k)$ utilisées dans ces deux modèles sont très similaires pour $k > \SI{0.3}{\h\per\Mpc}$.
...
Si ces modèles ne parviennent pas à distinguer l'effet du \lya{} de l'effet des HCD possédant une densité de colonne fixe, alors il y a peu d'espoirs qu'ils parviennent à le faire lorsque les HCD sont distribués dans une large gamme en $\log n_{\mathrm{HI}}$.
...
\paragraph{}
Le dernier test que nous présentons consiste à fixer les paramètres \lya{} lors de l'ajustement des modèles décrits dans le paragraphe précédent sur les mocks qui possédent des HCD avec une densité de colonne fixe, et ajuster le paramètre $L_{\textsc{HCD}}$. Puis, refaire l'ajustement avec le modèle de Rogers en relâchant les paramètres \lya{} et en utilisant la valeur de $L_{\textsc{HCD}}$ obtenue précédemment.
...
Nous pouvons remarquer que le modèle de Rogers lorsqu'il est ajusté sur les mocks avec les HCD possédant la plus faible densité de colonne donne des mesures de $L_{\textsc{HCD}}$ bien plus grandes que lorsqu'il est ajusté sur les mocks avec les HCD possédant une grande densité de colonne. Nous ne comprenons pas cet effet.
