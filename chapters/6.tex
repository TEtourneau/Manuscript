...
Nous faisons référence à ce modèle via le nom \emph{modèle C-G}. Le modèle décrit dans la section~\ref{subsec:model_donnees} est dénommé \emph{modèle de Rogers}.
...
L'avantage de cette méthode est qu'elle n'utilise pas le paramètre effectif $L_{\textsc{HCD}}$. Elle permet donc de modéliser des distributions de HCD avec une plus grande gamme en $\log n_{\textsc{HI}}$, là où le modèle utilisé pour analyser les données DR16 ne fonctionne plus très bien.
...
Premièrement, nous comparons l'ajustement des fonctions de corrélations estimées à partir du champ $\delta_F$, où les HCD ont été masqués. Ceci nous permet de faire une comparaison directe des deux modèles ajustés sur les mêmes fonctions de corrélations.
...
\paragraph{}
Nous comparons maintenant la mesure des paramètres \lya{} faite sur la corrélation \lya{}$\times$\lya{}, où les HCD sont masqués et ajustée avec le modèle de Rogers, à la mesure faite sur la corrélation \lya{}$\times$\lya{}, où aucun HCD n'est masqué et ajustée avec le modèle C-G.
...
De ce fait, les mesures de $b_{\mathrm{eff},\mathrm{Ly}\alpha}$ et  $\beta_{\mathrm{Ly}\alpha}$ faites sur les mocks eboss-0.2 sans masquages des HCD et avec le modèle C-G sont presque compatibles avec les mesures faites sur les mocks eboss-0.0.
