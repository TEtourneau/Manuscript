...
\textbf{Nous pouvons remarquer dans ce tableau que, comme pour les mocks eboss-0.0, la mesure des paramètres \lya{} dans la corrélation croisée \lya{}$\times$QSO est en tension avec la mesure faite sur l'auto-corrélation \lya{}$\times$\lya{}.
  ...
  Nous pouvons cependant remarquer que les quantités $b_{\mathrm{Ly}\alpha} + b_{\textsc{HCD}}$ et $b_{\mathrm{Ly}\alpha}\beta_{\mathrm{Ly}\alpha} + b_{\textsc{HCD}}\beta_{\textsc{HCD}}$ (équation~\ref{eq:bias_hcd_def}) mesurés dans les données DR16, ainsi que dans les corrélations \lya{}$\times$\lya{} et \lya{}$\times$QSO sont compatibles. Ces mesures sont présentées sur la figure~\ref{prov}.
}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{sum_bias}
