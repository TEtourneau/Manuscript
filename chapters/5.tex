...
Pour chacune des \num{100} réalisations produites, nous avons calculé les fonctions de corrélation \lya{}$\times$\lya{} et \lya{}$\times$QSO des raw mocks.
...
Toutes les fonctions de corrélation présentées ici, à l'exception de la fonction de corrélation à une dimension, sont estimées dans quatre bins en redshift. Ces bins sont les mêmes que ceux choisis pour analyser les données et déterminer $b_{\mathrm{Ly}\alpha}(z)$ et $\beta_{\mathrm{Ly}\alpha}(z)$ à utiliser pour la construction des mocks (voir chapitre~\ref{prov}). Ces bins sont : $[\num{0}\,;\num{2.35}]$, $[\num{2.35}\,;\num{2.65}]$, $[\num{2.65}\,;\num{3.05}]$ et $[\num{3.05}\,;\num{10}]$. Nous utilisons les mêmes bins pour faciliter la comparaison entre les mocks et les données.

\subsubsection{L'auto-corrélation \lya{}$\times$\lya{}}
La figure~\ref{fig:cf_eboss00_4bins} donne l'addition des \num{100} fonctions de corrélation \lya{}$\times$\lya{} des raw mocks dans chaque bin en redshift.
...
Les raw mocks sont en excellent accord avec le modèle de picca.
% \begin{figure}
%   \centering
...
Maintenant que nous avons vérifié que les raw mocks possèdent les bonnes fonctions de corrélation, nous pouvons les analyser après avoir appliqué \texttt{quickquasars}.
