...
Après l'apparition des premières étoiles, le rayonnement émis par celles-ci commence progressivement à réioniser l'hydrogène, qui était devenu neutre au moment de l'émission du CMB. Les dernières analyses estiment le redshift de la réionisation à $5 < z_{réionisation} < 9$ (\#prov citer Planck et ?)
...
Dans un tel cas, on peut calculer le membre de gauche de l'équation~\ref{eq:einstein}. Ce calcul, que nous ne détaillerons pas ici, est très bien détaillé dans \cite{CITE: Dodelson 2.1.2}. De plus, pour un fluide parfait, le tenseur énergie impulsion prend la forme
\begin{equation}
  T_{\mu \nu} =
  \begin{pmatrix}
    -\rho & 0 & 0 & 0 \\
...
\paragraph{Evolution de l'univers —}
...
\textbf{La matière baryonique peut être vue comme un gaz de galaxies n'interagissant entre elles  que via la gravitation.
% \textbf{De la même manière, la matière noire froide peut être vue comme des particules massives n'interagissant les unes avec les autres que via la gravitation.}
...
\begin{figure}
  ...
  \caption{Carte des fluctuations en température mesurées par le satellite Planck~\cite{CITE}. La zone grise délimite la zone contaminée par les émissions galactiques, qui est masquée dans l'analyse.}
  \label{fig:carte_cmb}
\end{figure}
\begin{figure}
  \centering
...
Au fur et à mesure que le temps s'écoule (le redshift diminue), le front d'onde dans le plasma photon-baryon se propage. Puis, à un redshift $z_{drag} \sim 1060$, les baryons se découplent des photons. La pression dans le milieu devient nulle, faisant chuter la vitesse du son à zéro. L'onde est alors gelée.
% Ainsi la surdensité de baryon ne se propage plus, et les photons, qui n'interagissent plus avec les baryons, se propagent librement. Il n'y a alors plus que la gravitation qui affecte les distributions de chaque espèce.
...
% Ainsi, même si la majeure partie des baryons tombent dans le puit de potentiel créé par la surdensité initiale, un second puit de potentiel, formé par des baryons et de la matière noire, subsiste en $R \sim \SI{150}{\Mpc}$. Aujourd'hui, les distributions de matière noire et des baryons sont quasiment identiques.
...
\begin{figure}
  ...
  \caption{Mesure de $H(z) / (1+z)$ à l'aide des BAO. Les points roses~\cite{CITE:Beutler2011}, noirs~\cite{CITE:Ross2015} et verts~\cite{CITE:Alam2017} présentent les mesures faites avec des galaxies. Le point rouge donne la mesure faite avec l'auto-corrélation du \lya{}~\cite{CITE:Agathe2019} et le point bleu la mesure faite avec la corrélation croisée \lya{}-QSO~\cite{CITE:Blomqvist2019}. La ligne noire montre la valeur donnée par la cosmologie de Planck~\cite{CITE:Planck2016}. Crédits : \cite{CITE:Agathe2019}}
  \label{fig:h_vs_z}
\end{figure}
...
% \end{figure}
\textbf{Les galaxies, comme tous les objets effondrés, se forment dans les endroits les plus denses de l'univers.
...
La fonction de corrélation associée au traceur $i$ est amplifiée par un facteur $b_{i}^2$. Il est donc avantageux de choisir un traceur avec un biais important, afin d'obtenir une fonction de corrélation avec une amplitude importante, et ainsi un rapport signal sur bruit plus grand. \textbf{La première détection des BAO a ainsi été faite avec les LRG (des galaxies rouges) dont la mesure du biais donne $b_{LRG} \sim 2$~\cite{CITE}.}
...
\end{equation}
où $f = \frac{d \ln{G}}{d \ln{a}}$ est le taux de croissance, et $\mu_k = \frac{\vec k \cdot \vec u}{k}$, où $\vec u$ est la direction de la ligne de visée.
Le vecteur $\vec k$ peut être décomposé comme $\vec k = \kpar{} \vec u + \vec \kperp{}$, où $\vec \kperp{}$ est perpendiculaire à la ligne de visée. La quantité $\mu_k$ vaut alors $\kpar{} / k$. 
...
\paragraph{}
La forêt \lya{} est donc une observable très avantageuse. Elle permet de sonder les contrastes de densité faibles en traçant le milieu intergalactique diffus. Elle permet aussi de sonder l'univers à grands redshifts grâce à la grande luminosité des quasars. De plus, chaque spectre  de quasar fournit un grand nombre de pixels d'absorption \lya{}. Enfin, le relevé spectroscopique de quasar \lya{} ne requiert pas d'être homogène, contrairement aux traceurs booléens. En effet, comme le \lya{} est senbible à la densité, via l'observation du flux transmis, il est possible de construire un contraste (équation~\ref{eq:delta_lya}) similaire au contraste de densité de la matière (équation~\ref{eq:contraste}). Ce n'est pas le cas des traceurs booléens car ces derniers ne renseignent pas sur la densité sondée. Les analyses BAO et RSD qui utilisent ces traceurs construisent alors la fonction de corrélation différemment, en ayant recours à l'estimateur de Landy-Szalay. Cette estimateur compare la distribution de traceurs observés à une distribution aléatoire. Il est donc important d'avoir un échantillon de traceur homogène.
...
% Comme expliqué dans la section précédente, chaque absorption observée dans la forêt \lya{} est supposée être causée par un absorbeur \lya{}. Cependant le milieu intergalactique n'absorbe pas uniquement en \lya{}. De nombreuses espèces, en plus de l'hydrogène, sont présentes, et peuvent causer de l'absorption.

% Par abus de langage, les métaux désignent les espèces qui possèdent un numéro atomique supérieur à 2. A la différence de l'hydrogène qui est d'origine primordiale\footnote{L'hydrogène et l'hélium sont formés durant la nucléo-synthèse primordiale.}, les métaux sont formés par les étoiles, et sont dispersés dans  par les supernoae. Chaque métal possède un certain nombre de transitions, qui peuvent absorber la lumière issue du quasar observé (\#prov donner le tableau des transisions utilisées dans picca ? Ou faire reference au tableau precedent?). Selon le redshift du métal, la raie d'absorption peut se trouver dans la forêt \lya{}. Si c'est le cas, la raie d'absorption causée par le métal va induire une fausse corrélation dans le calcul de la fonction de corrélation. Pour illustrer ce propos, supposons un métal situé à un redshift $z_{met}$. Ce métal produit une absorption à une longueur d'onde $\lambda_{met}$ dans son référentiel, et $\lambda_{obs} = \lambda_{met} (1+z_{met})$ dans le référentiel de l'observateur.
% Cette absorption est observée dans la forêt \lya{}. Elle est donc interprétée comme une absorption \lya{}, et le redshift de l'absorbeur est alors reconstruit à $z_{abs} = \frac{\lambda_{obs}}{\lambda_{\mathrm{Ly}\alpha}} - 1 \neq z_{met}$. Etant donné que le champ des métaux est corrélé avec le champ d'hydrogène neutre, cette erreur dans la détermination du redshift de l'absorbeur crée une fausse corrélation. 

Le \lya{} n'est pas le seul absorbeur présent dans le spectre des quasars. D'autres espèces peuvent être présentes le long de la ligne de visée, et ainsi causer de l'absorption. Par abus de langage, les métaux désignent les éléments qui possèdent un numéro atomique supérieur à 2. A la différence de l'hydrogène qui est d'origine primordiale\footnote{L'hydrogène et l'hélium sont formés durant la nucléo-synthèse primordiale.}, les métaux sont formés par les étoiles, puis sont dispersés via l'explosion des supernovae. Les métaux sont donc trouvés proches des environnements qui forment des étoiles : les galaxies.

Comme expliqué dans la section précédente, chaque absorption observée dans la forêt \lya{} est supposée être causée par un absorbeur \lya{}. Cependant, rien ne nous garanti que l'absorption observée n'est pas causée par un autre absorbeur, à un autre redshift. Etant donné que le champ des métaux est corrélé avec le champ d'hydrogène neutre, les raies d'absorptions des métaux sont corrélés avec celles du \lya. Supposons la présence d'un métal à un redshift $z_{met}$, absorbant dans son référentiel à une longueur d'onde $\lambda_{met}$. L'absorption produite est observée à $\lambda_{obs1} = \lambda_{met} ( 1 + z_{met})$, mais sera reconstruite à un redshift $z_{abs} = \frac{\lambda_{obs1}}{\lambda_{\mathrm{Ly}\alpha}} - 1 \neq z_{met}$. De plus, la présence d'hydrogène neutre au redshift du métal $z_{met}$ produit aussi une absorption, à une longueur d'onde $\lambda_{obs2} = \lambda_{\mathrm{Ly}\alpha}(1+z_{met})$. Ainsi, à chaque raie d'absorption de métal, interprétée comme de l'absorption \lya{}, est associée une autre raie d'absorption \lya{}. Ceci produit une fausse corrélation le long de la ligne de visée, à une séparation caractérisée par le rapport $\lambda_{met} / \lambda_{\mathrm{Ly}\alpha}$. Cette effet est très présent dans la fonction de corrélation à une dimension : lorsque l'on corrèle des pixels d'une même forêt, mais aussi dans la fonction de corrélation à trois dimensions le long de la ligne de visée ($\rperp{} < \SI{4}{\perh\Mpc}$). (\#prov faire référence à une figure qui montre la fonction de corrélation de DR16 ?)


% Est-ce que je donne la séparation en fonction de zmet ? Bof ca depend des distances ...
% En fonction des rapports des lambda ? Je peux montrer xi1D vs lambda1/lambda2

% C'est peut etre pas utile que je passe du temps a decrire ca, etant donne que j'analyse peu les donnees ? 

% Parler des DLAs et HCDs
% Parler des BALs ? (mentionner en quelques lignes) (mettre un spectre)

\paragraph{}
Un autre contaminant présent dans la forêt \lya{} sont les HCD (High Column Density). Les HCD sont des absorbeurs possédant une densité de colonne\footnote{La densité de colonne mesure la quantité de matière intégrée le long de la ligne de visée. Elle est mesurée en \si{atome\per\centi\meter\squared}.} supérieure à $\SI{1.6e17}{atome\per\centi\meter\squared}$ \cite{CITE Rogers2018}. Ces absorbeurs très denses produisent de très fortes absorptions. Ils correspondent à des sytèmes effondrés, comme le gas présent dans ou autour des galaxies. Leur fonction de corrélation est donc beaucoup plus structurée que celle du \lya{}. De plus, du fait des vitesses importantes du gas présent dans ces objets effondrés, le profile d'absorption des HCD est élargi, produisant une absorption au delà de la position physique de l'absorbeur. Ce profile d'absorption peut être modélisé par un profile de Voigt.
La figure~\ref{fig:exemple_dla} montre le spectre d'un quasar possédant un DLA, ainsi que le profile de voigt ajusté. Les DLA sont les HCD pour lesquels la densité de colonne est supérieure à \SI{2e20}{atome\per\centi\meter\squared}. Ces HCD sont tellement denses qu'ils peuvent être identifiés dans les spectres. Dans l'analyse~\ref{CITE DR16}, les DLA identifiés sont modélisés par un profile de Voigt, puis, tous les pixels pour lequels la fraction de flux transmis est inférieure à \num{0.80} sont masqués.
Les HCD pour lesquels la densité de colonne est inférieure à \SI{2e20}{atome\per\centi\meter\squared} ne sont pas identifiables. Ils ne peuvent donc pas être masqués. Cependant, ils doivent être traités différemment des absorptions moyennes du \lya{}. Le traitement des HCD est détaillé dans la section~\ref{prov}
\begin{figure}
  \centering
